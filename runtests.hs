...
testSuiteMin = testGroup "Text.Jasmine.Pretty"
    ...
    , testCase "MinIssue7"        caseMinIssue7
    ]
...
--srcIssue7="var hljs=new function(){function k(){if(k.called){return}k.called=true;var r=document.getElementsByTagName(\"pre\");for(var p=0;p<r.length;p++){}}}();"
srcIssue7="var hljs=new function(){function k(){if(k.called){return}}}();"
caseIssue7 =
  "Right (JSSourceElementsTop [JSVariables \"var\" [JSVarDecl (JSIdentifier \"hljs\") [JSLiteral \"new \",JSFunctionExpression [] [] (JSFunctionBody [JSSourceElements [JSFunction (JSIdentifier \"k\") [] (JSFunctionBody [JSSourceElements [JSIf (JSExpression [JSMemberDot [JSIdentifier \"k\"] (JSIdentifier \"called\")]) (JSStatementBlock (JSStatementList [JSReturn [JSLiteral \"\"]]))]])]]),JSArguments []]]])"
  @=? (showStrippedMaybe $ parseProgram srcIssue7)
caseMinIssue7 =
  testMinify "var hljs=new function(){function k(){if(k.called)return}}()" srcIssue7

caseIssue7file =
  do
     x <- readFile ("./test/parsingonly/issue7.js")
     y <- readFile ("./test/parsingonly/issue7.js")
     let x' = trim x
     testMinify x' y
